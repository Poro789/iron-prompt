...
### 变更
- 休息计时重构：窄条显示在卡片顶部（不替换组内容），倒计时到 0 后继续向上计（超时），去掉 ±15s 按钮，只留「跳过」；实际休息时长自动记录到 set.restAfter
- 完成按钮改为二态：✗ 未完成（红，默认）→ ✓ 完成（绿）→ ✗；去掉 ○ 未记录态（旧数据 null 自动迁移为 false）
- 导航重构：底部三 tab 删除，改为右上角 ☰ 汉堡抽屉（含 A/B 日切换 + 三个视图）；版本号移到顶栏中央
- 卡片布局：动作名 + 标记第一行、肌群第二行（防换行错乱）；去掉组级备注框（只保留动作级）；卡片 min-height 固定防跳动
- 设置融合：「记录」和「AI 分析背景」合并为一个自由文本「训练备注 / 背景」
- 新增：当日总进度条（卡片顶部细线）、切组自动 scrollTo top、所有组 ✗ 时显示「跳过此动作」按钮

## [0.8.0]

### 变更
- 今日训练改为全屏一次一组：一屏只显示当前组（大数字输入 + 备注 + RPE 步进 + 三态完成按钮），左右箭头切组
- 完成按钮三态循环：○ 未记录 → ✓ 完成（绿）→ ✗ 未完成（红）→ ○
- 去掉重量/次数/时长的步进按钮，改为直接输入（重量小数、次数/时长整数）
- 组间休息集成进全屏卡片：确认一组后卡片切到休息态（大倒计时 + 跳过/±15s），结束自动切下一组
- 新增组级备注（set.note）与动作级备注（exercise.note），随日志落盘并导出给 AI
- 历史详情显示备注
